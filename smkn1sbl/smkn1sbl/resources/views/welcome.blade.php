...
@section('content')
    ...
    <div class="relative z-10 px-6 mt-6 xl:-mt-20">
        <div class="max-w-4xl mx-auto bg-white/90 md:bg-white/80 backdrop-blur-xl border border-white/60 rounded-3xl shadow-2xl shadow-skblue-900/20 px-5 py-6 md:px-10 md:py-8 grid sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <a href="{{ route('pakar.consultation.create') }}"
               class="group flex flex-col items-center text-center gap-2 rounded-2xl px-4 py-4 sm:py-5 hover:bg-skblue-50 transition">
                ...
                <p class="font-display font-semibold text-skblue-900 text-sm">Konsultasi Jurusan</p>
                <p class="text-xs text-slate-500">Sistem pakar rekomendasi</p>
            </a>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @forelse($galleries ?? [] as $item)
                    <div class="rounded-2xl overflow-hidden h-48 bg-skblue-100 reveal-zoom">
                        <img src="{{ asset('storage/'.$item->image_path) }}" class="w-full h-full object-cover hover:scale-105 transition duration-300" alt="{{ $item->title }}">
                    </div>
                @empty
                    @for($i = 0; $i < 4; $i++)
